...
public class EmailSendUtils {
    ...
    private EmailSendUtils() {
        // EmailSendUtils.config(EmailSendUtils.SMTP_163(false), "18785779149@163.com", "MTVLPWBJUFTJUEVM");
        EmailSendUtils.config(EmailSendUtils.SMTP_QQ(false), "468264345@qq.com", "ldufdvxeackubidi");
    }
    ...
    public static EmailSendUtils subject(String subject) {
        ...
        EmailSendUtils.msg = new MimeMessage(session);
        try {
            EmailSendUtils.msg.setSubject(subject, "UTF-8");
        } catch (Exception e) {
            log.error(TAG, e + "");
        }
     ...
     */
    public EmailSendUtils from(String nickName) {
     ...
     */
    public EmailSendUtils from(String nickName, String from) {
        try {
    ...
    public EmailSendUtils replyTo(String... replyTo) {
    ...
    public EmailSendUtils to(String... to) throws MessagingException {
    ...
    public EmailSendUtils cc(String... cc) throws MessagingException {
    ...
    public EmailSendUtils bcc(String... bcc) throws MessagingException {
    ...
    private EmailSendUtils addRecipients(String[] recipients, Message.RecipientType type) throws MessagingException {
        String result = Arrays.asList(recipients).toString().replace("(^\\[|\\]$)", "").replace(", ", ",");
        msg.setRecipients(type, InternetAddress.parse(result));
        return this;
    }

    private EmailSendUtils addRecipient(String recipient, Message.RecipientType type) throws MessagingException {
        msg.setRecipients(type, InternetAddress.parse(recipient.replace(";", ",")));
        return this;
    }

    public EmailSendUtils text(String text) {
        this.text = text;
        return this;
    }

    public EmailSendUtils html(String html) {
        this.html = html;
        return this;
    }
    ...
    public EmailSendUtils attach(File file, String fileName) {
        attachments.add(createAttachment(file, fileName));
        return this;
    }

    public EmailSendUtils attachURL(URL url, String fileName) {
    ...
    private MimeBodyPart createAttachment(File file, String fileName) {
        ...
        try {
            ...
            attachmentPart.setFileName(null == fileName ? MimeUtility.encodeText(fds.getName()) :
    ...
    private MimeBodyPart createURLAttachment(URL url, String fileName) {
        ...
        try {
            ...
            attachmentPart.setFileName(null == fileName ? MimeUtility.encodeText(fileName) :
                    MimeUtility.encodeText(fileName));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /**
     * 发送邮件，如果超时还没有收到，用户再次调用
     * @param emailAddress
     * @param content
     */
    public static void sendEmailCode(String emailAddress, String content) {
        try {
            EmailSendUtils.subject("邮箱验证码")
                    .from("摸鱼君-统一用户中心")
                    .text(content)
                    .to(emailAddress)
                    .send();
